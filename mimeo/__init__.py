...
def syscall(cmd, verbose=False):
    ...
    try:
        ...
        print('The following command failed with exit code',
...
def set_paths(adir=None, bdir=None, afasta=None,
              bfasta=None, outdir=None, outtab=None,
              gffout=None, suppresBdir=False, runtrf=None):
    if not adir:
        # Make temp directory
...
def import_Align(infile=None, prefix=None, minLen=100, minIdt=95):
    ...
    df = df.sort_values(['tName', 'tStart', 'tEnd', 'tStrand'],
...
def trfFilter(alignDF=None, tempdir=None, adir=None, prefix=None,
              TRFpath='trf', tmatch=2, tmismatch=7, tdelta=7, tPM=80,
              tPI=10, tminscore=50, tmaxperiod=50, maxtandem=40):
    ...
    # Run TRF
    cmds = list()
    trf_cmd = ' '.join([str(TRFpath), quote(AlnFasta), str(tmatch),
                        str(tmismatch), str(tdelta), str(tPM), str(tPI),
                        str(tminscore), str(tmaxperiod), '-m', '-h', '-ngs >',
                        quote(AlnFasta + '.dat')]
                       )
    # cmds.append(' '.join(["echo 'Run TRF as: ' >&2"]))
    # cmds.append(' '.join(["echo '" + trf_cmd + "' >&2"]))
    cmds.append(trf_cmd)
    maskfile = '.'.join(['raw_A_genome_hits.fa', str(tmatch), str(tmismatch),
                         str(tdelta), str(tPM), str(tPI), str(tminscore),
                         str(tmaxperiod), 'mask']
                        )
    ...
    # Reindex
    alignments = alignments.sort_values(['tName', 'tStart', 'tEnd', 'tStrand'],
                                        ascending=[True, True, True, True])
...
def trfFasta(fasta=None, outfile=None, TRFpath='trf', tmatch=2,
             tmismatch=7, tdelta=7, tPM=80, tPI=10, tminscore=50,
             tmaxperiod=50, maxtandem=40, verbose=True, keeptemp=False):
    # Run TRF
    cmds = list()
    trf_cmd = ' '.join([str(TRFpath), quote(fasta), str(tmatch),
                        str(tmismatch), str(tdelta), str(tPM), str(tPI),
                        str(tminscore), str(tmaxperiod), '-m', '-h', '-ngs >',
    ...
    # cmds.append(' '.join(["echo '" + trf_cmd + "' >&2"]))
    cmds.append(trf_cmd)
    maskfile = '.'.join([os.path.basename(fasta), str(tmatch), str(tmismatch),
                         str(tdelta), str(tPM), str(tPI), str(tminscore),
                         str(tmaxperiod), 'mask']
                        )
...
def writetrf(alignDF=None, outtab=None):
    ...
    outfile = outtab + '.trf'
    with open(outfile, 'w') as handle:
        handle.write('\t'.join(['#name1', 'strand1', 'start1', 'end1', 'name2',
                                'strand2', 'start2+', 'end2+', 'score',
                                'identity' + '\n'])
                     )
        for index, row in alignDF.iterrows():
            handle.write('\t'.join([row['tName'], row['tStrand'],
                                    row['tStart'], row['tEnd'], row['qName'],
                                    row['qStrand'], row['qStart'], row['qEnd'],
                                    row['score'], row['pID'] + '\n'])
                         )
    return outfile
...
def writeGFFlines(alnDF=None, chrlens=None, ftype='BHit'):
    ...
    if chrlens:
        for name, maxlen in chrlens:
            yield ' '.join(["##sequence-region", str(name), '1',
                            str(maxlen) + '\n']
                           )
    yield '\t'.join(['##seqid', 'source', 'type', 'start', 'end', 'score',
...
def map_LZ_cmds(lzpath="lastz", pairs=None, minIdt=95, minLen=100,
                hspthresh=3000, outfile=None, verbose=False):
    '''
    ...
    for A, B in pairs:
        ...
        cmds.append(' '.join([lzpath, quote(t_file), quote(q_file),
                              "--entropy --format=general:name1,strand1,start1,end1,length1,name2,strand2,start2+,end2+,length2,score,identity --markend --gfextend --chain --gapped --step=1 --strand=both --hspthresh=" + str(hspthresh),
                              "--output=" + temp_outfile,
                              "--verbosity=" + str(verb)])
                    )
        # Scrub % symbols
...
def xspecies_LZ_cmds(lzpath="lastz", bdtlsPath="bedtools", Adir=None,
                     Bdir=None, pairs=None, outtab=None, outgff=None,
                     minIdt=60, minLen=100, hspthresh=3000, minCov=5,
                     AchrmLens=None, reuseTab=False, label="B_repeats",
    ...
    if not reuseTab or not os.path.isfile(outtab):
        ...
        for A, B in pairs:
            ...
            # New Header = name1,strand1,start1,end1,name2,strand2,start2+,end2+,score,identity
            # Sort filtered file by chrom, start, stop
            cmds.append(' '.join(["awk '!/^#/ { print; }'", temp_outfile,
                                  "| awk -v minLen=" + str(minLen),
                                  "'0+$5 >= minLen {print ;}' | awk -v OFS='\\t' -v minIdt=" + str(minIdt),
    ...
    cmds.append(' '.join(["awk -v OFS='\\t' '!/^#/ {print $1,$3,$4;}'",
                          outtab, ">", temp_bed])
                )
    # Remove spaces
    cmds.append(' '.join(["sed -i '' -e 's/ //g'", temp_bed]))
    # Sort filtered bed file by chrom, start, stop
    cmds.append(' '.join(["sort -k 1,1 -k 2n,3n", temp_bed, ">",
                          temp_bed_sorted])
                )
    ...
    cmds.append(' '.join([bdtlsPath, "genomecov -bg -i", temp_bed_sorted,
                          "-g", AchrmLens,
                          "| awk -v OFS='\\t' -v cov=" + str(minCov),
                          "'0+$4 >= cov {print ;}' >", temp_bed])
                )
    # Re-sort
    cmds.append(' '.join(["sort -k 1,1 -k 2n,3n", temp_bed,
                          ">", temp_bed_sorted])
    ...
    cmds.append(' '.join([bdtlsPath, "merge -i", temp_bed_sorted, ">",
                          temp_bed])
                )
    # Write GFF header
    ...
    cmds.append(' '.join(["awk -v minLen=" + str(minLen),
                          "'{ if($3 - $2 >= minLen) print ;}'",
                          temp_bed,
...
def self_LZ_cmds(lzpath="lastz", bdtlsPath="bedtools", splitSelf=False,
                 Adir=None, Bdir=None, pairs=None, outtab=None, outgff=None,
                 minIdt=60, minLen=100, hspthresh=3000, minCov=3, intraCov=5,
                 AchrmLens=None, reuseTab=False, label="Self_repeats",
                 prefix=None, verbose=False):
    '''
    ...
    if not reuseTab or not os.path.isfile(outtab):
        ...
        for A, B in pairs:
            if A != B or not splitSelf:
                ...
                q_file = B
                q_name = os.path.splitext(os.path.basename(B))[0]
                temp_outfile = '_'.join(["temp", q_name,
                                         "onto", t_name, ".tab"]
                                        )
                # Compose LASTZ command
                cmds.append(' '.join([lzpath, quote(t_file), quote(q_file),
                                      ...
                                      + str(hspthresh), "--output="
                                      + temp_outfile, "--verbosity="
                                      + str(verb)])
                            )
                # Scrub % symbols
                cmds.append(' '.join(["sed -i '' -e 's/%//g'", temp_outfile]))
                ...
                q_file = B
                q_name = os.path.splitext(os.path.basename(B))[0]
                temp_outfile = '_'.join(["temp", q_name, "onto",
                                         t_name, ".tab"]
                                        )
                # Compose LASTZ command
                cmds.append(' '.join([lzpath, quote(t_file), quote(q_file),
    ...
    cmds.append(' '.join([bdtlsPath, "genomecov -bg -i", temp_bed_sorted, "-g",
                          AchrmLens, "| awk -v OFS='\\t' -v cov="
                          + str(minCov), "'0+$4 >= cov {print ;}' >",
                          temp_bed]
                         )
                )
    # Re-sort
    ...
    if splitSelf:
        if reuseTab and not os.path.isfile(outtab_intra) and os.path.isfile(outtab):
            ...
            # Port filtered hits to bed format: "name1,start1,end1"
            cmds.append(' '.join(["awk -v OFS='\\t' '!/^#/ {print $1,$3,$4;}'",
                                  outtab_intra, ">", temp_bed_intra]
                                 )
                        )
            # Remove spaces
            cmds.append(' '.join(["sed -i '' -e 's/ //g'", temp_bed_intra]))
            # Sort filtered bed file by chrom, start, stop
            cmds.append(' '.join(["sort -k 1,1 -k 2n,3n", temp_bed_intra, ">",
            ...
            cmds.append(' '.join([bdtlsPath, "genomecov -bg -i",
                                  temp_bed_sorted_intra, "-g", AchrmLens,
                                  "| awk -v OFS='\\t' -v cov=" + str(intraCov),
                                  "'0+$4 >= cov {print ;}' >", temp_bed_intra]
                                 )
                        )
            # Re-sort
            cmds.append(' '.join(["sort -k 1,1 -k 2n,3n", temp_bed_intra, ">",
            ...
            # Merge end-to-end annotations
            cmds.append(' '.join([bdtlsPath, "merge -i", temp_bed_sorted_intra,
                                  ">", temp_bed_intra]
                                 )
                        )
            # Filter orphan fragments < xx && Create GFF3 file
            cmds.append(' '.join(["awk -v minLen=" + str(minLen),
                                  "'{ if($3 - $2 >= minLen) print ;}'",
                                  temp_bed_intra, "| awk -v OFS='\\t' 'BEGIN{i=0}{i++;}{j= sprintf(\"%05d\", i)}{print $1,\"mimeo-self\",\""
                                  + str(label) + "_intra"
                                  + "\",$2,$3,\".\",\"+\",\".\",\"ID="
                                  + str(prefix) + "_\"j ;}' >>", quote(outgff)]
                                 )
