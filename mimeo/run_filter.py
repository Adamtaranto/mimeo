...
def mainArgs():
    parser = argparse.ArgumentParser(description='Filter SSR containing sequences from fasta library of repeats.',
                                     prog='mimeo-filter')
    # Input options
    parser.add_argument('--infile',type=str,required=True,help='Name of directory containing sequences from A genome.')
    # Output options
    parser.add_argument('-d', '--outdir',type=str,default=None,help='Write output files to this directory. (Default: cwd)')
    parser.add_argument('--outfile',type=str,default=None,help='Name of alignment result file.')
    ...
    parser.add_argument('--tmaxperiod',type=int,default=50,help='TRF maximum period size to report. Note: Setting this score too high may exclude some LTR retrotransposons. Optimal len to exclude only SSRs is 10-50bp.')
    parser.add_argument('--maxtandem',type=float,default=40,help='Max percentage of a sequence which may be masked by TRF. If exceeded, element will be discarded.')
...
def main():
    ...
    if not args.outfile:
        outname = os.path.splitext(
            os.path.basename(args.infile))[0] + '_filtered.fa'
    else:
    ...
    # Run filter
    mimeo.trfFasta(fasta=infile, outfile=outfile, TRFpath=args.TRFpath,
                   tmatch=args.tmatch, tmismatch=args.tmismatch,
                   tdelta=args.tdelta, tPM=args.tPM, tPI=args.tPI,
                   tminscore=args.tminscore, tmaxperiod=args.tmaxperiod,
                   maxtandem=args.maxtandem, verbose=args.verbose,
