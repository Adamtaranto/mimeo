...
def mainArgs():
    ...
    parser.add_argument('--minCov',type=int,default=3,help='Minimum depth of aligned segments to report repeat feature.')
    parser.add_argument('--hspthresh',type=int,default=3000,help='Set HSP min score threshold for LASTZ.')
    parser.add_argument('--intraCov',type=int,default=5,help='Minimum depth of aligned segments from same scaffold to report feature. Used if "--strictSelf" mode is selected.')
    parser.add_argument('--strictSelf',action="store_true",help='If set process same-scaffold alignments separately with option to use higher "--intraCov" threshold. Sometime useful to avoid false repeat calls from staggered alignments over SSRs or short tandem duplication.')
...
def main():
    ...
    adir_path, bdir_path, outdir, outtab, gffout, tempdir = mimeo.set_paths(
        adir=args.adir, afasta=args.afasta, outdir=args.outdir,
        outtab=args.outfile, gffout=args.gffout, suppresBdir=True)
    ...
    cmds = mimeo.self_LZ_cmds(lzpath=args.lzpath, bdtlsPath=args.bedtools,
                              pairs=pairs, Adir=adir_path, Bdir=bdir_path,
                              outtab=outtab, outgff=gffout, minIdt=args.minIdt,
                              minLen=args.minLen, hspthresh=args.hspthresh,
                              minCov=args.minCov, intraCov=args.intraCov,
                              splitSelf=args.strictSelf, AchrmLens=lenPathA,
                              reuseTab=args.recycle, label=args.label,
                              prefix=args.prefix)
    # Run alignments
